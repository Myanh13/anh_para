...
//san pham lien quan
    useEffect(() => {
        const fetchRooms = async () => {
          try {
            const response = await fetch(`https://datn-7xip.onrender.com/homestaylienquan/${id}`);
            const data = await response.json();
      ...
      useEffect(() => {
        if (!storedUser) {
            ...
            const fetchUserData = async () => {
                try {
                    const response = await fetch(`https://datn-7xip.onrender.com/user/${storedUser.id_user}`, {
                        method: 'GET',
                        headers: {
...
useEffect(() => {
  ...
  if (isNotificationVisible) {
    ...
    setTimeLeft(8); // Đặt lại bộ đếm về 5 giây
    ...
    if (homestayCT) { // Kiểm tra nếu dữ liệu homestay đã tải
      try {
        ...
        if (!isFavorite) {
          ...
          // Hiển thị thông báo sản phẩm được thêm vào danh sách yêu thích
          setNotificationVisible(true);
          setTimeLeft(5); // Đặt lại thời gian đếm ngược
        } else {
          // Hiển thị thông báo sản phẩm đã có trong danh sách yêu thích
          setNotificationVisible(true);
          setTimeLeft(5); // Đặt lại thời gian đếm ngược
        }
      } catch (error) {
        console.error("Lỗi khi xử lý dữ liệu yêu thích:", error);
        setNotificationVisible(true); // Hiển thị thông báo lỗi
        setTimeLeft(5);
      }
    }
  };
...
const handleSubmit = async (e) => {
    ...
    const phoneRegex = /^0\d{9}$/; // Chỉ cho phép 10 chữ số
    ...
    try {
        const response = await axios.post('https://datn-7xip.onrender.com/booking/homestay', bookingData);
        console.log('Đặt phòng thành công:', response.data.message);
        alert('Đặt phòng thành công!');
        navigate('/')
        console.log('dữ liệu nhận về', bookingData)
                                                            ...
                                                            <div className="col-sm-12 col-xs-12">
                                                                <div className="pro-datepicker t-datepicker">
                                                                        ...
                                                                        <div className="pro-form">
                                                                            ...
                                                                            <div className="t-check-in">
                                                                                <DatePicker
                                                                                    ...
                                                                                    minDate={new Date()}  // Đảm bảo không thể chọn ngày quá khứ
                                                                                />
                                                                    ...
                                                                    <div className="pro-item pro-when pro-checkout">
                                                                        <div className="pro-form">
                                                                            ...
                                                                            <div className="t-check-out">
                                                                                <DatePicker
                                                                                    ...
                                                                                    minDate={checkInDate}  // Ngày trả phòng không thể nhỏ hơn ngày nhận phòng
                                                                                />
                                                                        ...
                                                                        <input type="hidden" name="entry.1366726942" value={formatDateForInput(checkInDate)} />
                                                                        <input type="hidden" name="entry.1192184549" value={formatDateForInput(checkOutDate)} />
                                                                </div>
                                                            </div>
                                                            <div className="col-sm-12 col-xs-12">
...
{isNotificationVisible && (
  <>
    ...
    <div className="notification_thongbao_user" style={{ display: 'flex' }}>
      ...
      <p>{timeLeft > 0 ? 
        (existingFavorites.some(item => item.id_homestay === homestayCT.id_homestay) ? 
